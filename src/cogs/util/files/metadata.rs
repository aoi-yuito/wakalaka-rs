...
pub enum ArtificialIntelligence {
    A1111,
    NovelAI,
    Unknown,
}

impl BitOr for ArtificialIntelligence {
    type Output = Self;

    fn bitor(self, rhs: Self) -> Self::Output {
        match (self, rhs) {
            (Self::A1111, Self::NovelAI) => Self::A1111 | Self::NovelAI,
            (Self::NovelAI, Self::A1111) => Self::A1111 | Self::NovelAI,
            (Self::A1111, Self::Unknown) => Self::A1111 | Self::Unknown,
            (Self::Unknown, Self::A1111) => Self::A1111 | Self::Unknown,
            (Self::NovelAI, Self::Unknown) => Self::NovelAI | Self::Unknown,
            (Self::Unknown, Self::NovelAI) => Self::NovelAI | Self::Unknown,
            (Self::A1111, Self::A1111) => Self::A1111,
            (Self::NovelAI, Self::NovelAI) => Self::NovelAI,
...
impl ArtificialIntelligence {
    pub fn new_jpg(str: String) -> Self {
        //ExifIFD:UserComment applies to both
        if str.contains("ExifIFD:UserComment") {
            return Self::A1111 | Self::NovelAI;
        } else {
            return Self::Unknown;
        }
    }

    pub fn new_png(str: String) -> Self {
        match str.as_str() {
            "PNG:parameters" => Self::A1111,
            "PNG:Comment" => Self::NovelAI,
            _ => Self::Unknown,
        }
    }

    fn new_json_png(json: &LinkedHashMap<String, String>) -> Self {
...
pub struct FileMetadata {
    ...
    pub vae_hash: Option<String>, // A1111, ComfyUI
}
...
impl FileMetadata {
    fn new(json: LinkedHashMap<String, String>) -> Self {
        let ai = ArtificialIntelligence::new_json_png(&json);
        match ai {
            ArtificialIntelligence::A1111 => Self::new_a1111(json),
            ArtificialIntelligence::NovelAI => Self::new_novelai(json),
            ArtificialIntelligence::Unknown => Self::default(),
        }
    }
    ...
    fn new_a1111(json: LinkedHashMap<String, String>) -> Self {
        ...
        let steps = split_steps_[0].parse::<u32>().unwrap();
    ...
    pub async fn attachment_metadata(ctx: &Context, http: Arc<Http>, msg: &Message) {
        ...
        if channel_id != ChannelId::from(config.metadata_channel_id) {
            return;
        }
        ...
        if metadata_read_res_ok {
            ...
            let message_reactive = http
                .get_message(channel_id, message_id)
                .await
                .expect("Failed to get message");
            message_reactive
                .react(&http, mag_right.clone())
                .await
                .expect("Failed to react to message");

            let _ = event::message::send_dm_if_embed_attachment_reactive(
    ...
    async fn read_attachment_metadata(
        ...
        if files::is_attachment_jpeg(attachment) {
