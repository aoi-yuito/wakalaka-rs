...
pub(crate) async fn delwarn(
    ctx: Context<'_>,
    #[description = "Case identifier of warning to delete."] id: i32,
) -> Result<(), Error> {
    let pool = &ctx.data().pool;

    if id < 1 {
        let message = format!("Sorry, but you can't delete warning(s) with case ID less than 1.");
    ...
    let message = format!("I've deleted warning with case identifier of `{id}`.");
    let _ = ctx.reply(message).await;
