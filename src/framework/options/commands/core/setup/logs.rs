...
pub async fn logs(
    ctx: Context<'_>,
    #[description = "The channel to log messages in."] channel: Channel,
) -> Result<(), Error> {
    ...
    for guild_channel in guild_channels {
        ...
        let result = match guilds::update_guilds_set_logs_channel_id(
            guild_channel_id,
            guild_id,
            pool,
        )
        .await
        {
            Ok(_) => {
                ...
                Ok(format!("Configured {guild_channel_mention} to be for logging."))
            }
            Err(why) => {
                error!("Failed to configure #{guild_channel_name} for logging: {why:?}");
                Err(format!(
                        "An error occurred whilst configuring {guild_channel_mention} to be for logging."
                    ))
            }
        };
