...
pub async fn welcome(
    ctx: Context<'_>,
    #[description = "The channel to welcome users in."] channel: Channel,
) -> Result<(), Error> {
    ...
    for guild_channel in guild_channels {
        ...
        let result =
            match guilds::update_guilds_set_welcome_channel_id(guild_channel_id, guild_id, pool)
                .await
            {
                Ok(_) => {
                    info!("Configured #{guild_channel_name} for welcoming.");
                    Ok(format!(
                        "Configured {guild_channel_mention} to be for welcoming."
                    ))
                }
                Err(why) => {
                    error!("Failed to configure #{guild_channel_name} for welcoming: {why:?}");
                    Err(format!(
                        "An error occurred whilst configuring <#{guild_channel_id}> for welcoming."
                    ))
                }
            };
