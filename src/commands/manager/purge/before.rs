...
/// Delete a number of messages before specified.
pub(super) async fn before(
    ctx: Context<'_>,
    #[description = "Message to delete before."] message: Message,
    ...
    let deletable_msgs = GetMessages::default().before(msg_id).limit(count);
